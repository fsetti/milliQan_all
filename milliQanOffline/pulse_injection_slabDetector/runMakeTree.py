...
sys.path.insert(0,'/net/cms26/cms26r0/milliqan/milliqanOffline/')
import argparse
import findEvents
...
def parse_args():
	...
	parser.add_argument("-g","--gridPoint",help="Mass charge grid point",type=str)
	args = parser.parse_args()
	#print args
	return args

def main(inFiles,LPF,exe,pulseInject,signalInject,DRS,gridPoint):
    for inFileText in inFiles:
        ...
        for inFile in inFilesTextExpanded:
            args = [exe,inFile,"-1","None",gridPoint,"-1","-1","-1000","-1000","0","0","0",str(int(LPF)),str(int(pulseInject)),str(float(signalInject)),str(int(DRS))]
            print " ".join(args)
            call(args)
