...
void load_chanMap(){
	//slabs
	chanMap.push_back(	{-1,-1,0,1} );	//second pmt for each panel
	chanMap.push_back(	{-1,-1,1,1} );	//second pmt for each panel
	chanMap.push_back(	{-1,-1,2,1} );	//second pmt for each panel
	chanMap.push_back(	{-1,-1,3,1} );	//second pmt for each panel
	chanMap.push_back(	{-1,-1,4,1} );	//second pmt for each panel
	chanMap.push_back(	{-1,-1,5,1} );	//second pmt for each panel
	chanMap.push_back(	{-1,-1,6,1} );	//second pmt for each panel
	chanMap.push_back(	{-1,-1,7,1} );	//second pmt for each panel
}
...
vector< vector<int> > straightPaths = {{0,1,2,3,4,5,6,7}};
...
void make_tree(TString fileName, int eventNum, TString tag, TString grid_point, float rangeMinX,float rangeMaxX, float rangeMinY,float rangeMaxY, int displayPulseBounds, int onlyForceChans,int runFFT,int applyLPFilter,int injectPulses, float injectSignalQ,int runDRS, set<int> forceChan){
    ...
    if (runDRS){
		//if ( grid_point != "" ) milliqanOfflineDir+="DRS_slabSim/"+grid_point+"/";
		if ( grid_point != "" ) milliqanOfflineDir ="/data1/milliqan/DRS_slabSim/"+grid_point+"/";
		else milliqanOfflineDir+="DRS_slabSim/";
    }
	...
	std::vector<int> straightPath = straightPaths[0];
	fileID = fileIDDRS;
...
void prepareOutBranches(){
    ...
    TBranch *  b_chan_muDist = outTree->Branch("chan_muDist",chan_muDist,"chan_muDist[8]/F");
    TBranch *  b_chan_trueNPE = outTree->Branch("chan_trueNPE",chan_trueNPE,"chan_trueNPE[8]/I");
...
void loadBranchesMilliDAQ_slabSim(){
	waves.clear();
    ...
    for(int ic=0;ic<numChan;ic++){
		TString ch_num = std::to_string(ic+1);
